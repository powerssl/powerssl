...
import (
	...
	"powerssl.io/powerssl/internal/app/apiserver/user/endpoint"
...
func decodeGRPCUser(user *apiv1.User) (*api.User, error) {
	createTime, err := types.TimestampFromProto(user.GetCreateTime())
	if err != nil {
		return nil, err
	}
	updateTime, err := types.TimestampFromProto(user.GetUpdateTime())
	if err != nil {
		return nil, err
	}
	return &api.User{
...
func encodeGRPCUser(user *api.User) (*apiv1.User, error) {
	createTime, err := types.TimestampProto(user.CreateTime)
	if err != nil {
		return nil, err
	}
	updateTime, err := types.TimestampProto(user.UpdateTime)
	if err != nil {
		return nil, err
	}
	return &apiv1.User{
