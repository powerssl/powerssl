...
mappings=Mgoogle/api/annotations.proto=google.golang.org/genproto/googleapis/api/annotations
...
PATH="$(dirname "$(gobin -m -p github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-grpc-gateway)"):$PATH"
...
while IFS= read -r -d '' file; do
  for path in $(jq -r '.paths | to_entries[] | select(.key | test("=")) | .key' <"$file"); do
