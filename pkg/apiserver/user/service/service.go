...
import (
	...
	"powerssl.io/pkg/apiserver/user/model"
...
func (bs basicService) Create(ctx context.Context, user *api.User) (*api.User, error) {
	...
	server := model.NewUserFromAPI(user)
...
func (bs basicService) Delete(ctx context.Context, name string) error {
	...
	user, err := model.FindUserByName(name, db)
	if err != nil {
		return err
	}
...
func (bs basicService) List(ctx context.Context, pageSize int, pageToken string) ([]*api.User, string, error) {
	...
	if err := db.Limit(pageSize + 1).Offset(offset).Find(&users).Error; err != nil {
		return nil, "", err
	}
